...
void process_request(TCPRequestChannel* rc, Request* r){
	if (r->getType() == DATA_REQ_TYPE){
		...
		cerr << "Recieve NEWCHAN request which is invalid with sockets" << endl;
	}else{
...
int main(int argc, char *argv[]){
    ...
    TCPRequestChannel acceptingChan("", port);
    ...
    while(true) {  // main accept() loop
        ...
        int client_socket = accept(acceptingChan.getfd(), (struct sockaddr *)&their_addr, &sin_size);
        if (client_socket == -1) {
            perror("accept");
            break;
            continue;
        }
        thread t(handle_process_loop, new TCPRequestChannel(client_socket));
        t.detach();
    }
}
